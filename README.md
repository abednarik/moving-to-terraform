...
small but similar environment to what we have when we start working in this project.
...
In this case, *default* is set since we plan to use the same AMI in all environments and clusters,
is probably the few variables we want to have exactly the same in all environments.
...
Next we need to set some security groups. Terraform has plenty of resources. The way to define one
...
is the name we want to assign to to later reference this in our Terraform configuration. finally
...
this code for different environment, just changing a simple variable.
Security Group ins straight forward, we allow SSH and HTTP public traffic and everything is inside
our vpc *${var.vpc_id}*
...
After that we need to create a ELB.  Here there are a few things to have a look:
...
instance is triggered in a autoscaling action.
...
everywhere.
This is just fine when for this example, but I *highly recommend* only allowing ssh from inside
...
- Create a .tfvars file with your current data. I always use the environment name for this file, to reuse
the same Terraform configuration for multiple environments.
This is the one created for this guide. Just replace this values with your ouw data.
...
- Verify Terraform code and show what resources will be created
...
Nginx, in theu user data you will install your desired software like SaltStack, Chef or Puppet
set a role. Here is a small snippet on how we do it with Saltstack.
